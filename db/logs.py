...
class _VnTimeFormatter(logging.Formatter):
    ...
    def formatTime(self, record, datefmt=None):  # noqa: N802 (ten cua thu vien)
        ...
        return t.strftime(datefmt or _DATE_FORMAT)
...
def level_from_env() -> int:
    ...
    """
    raw = (os.environ.get("LOG_LEVEL") or DEFAULT_LEVEL).strip().upper()
    level = logging.getLevelName(raw)
    if isinstance(level, int):
        return level
    print(f"LOG_LEVEL={raw!r} is not valid, falling back to {DEFAULT_LEVEL}", file=sys.stderr)
...
def get_logger(name: str | None = None) -> logging.Logger:
    ...
    if not root.handlers:
        ...
        h.setFormatter(_VnTimeFormatter(_FORMAT, _DATE_FORMAT))
        root.addHandler(h)
