"""Bounded real-helper/LB/two-proxy MOCK test; no application DB/provider writes."""
import asyncio
...
from app.services.token_logger import token_usage_context, get_usage_context, build_token_usage_log
...
async def main():
    run_id = 'law-lb-' + secrets.token_hex(6)
    ...
    destination = ARTIFACTS / (run_id + '.json')
    key = None
    ...
    async def guarded_completion(**kwargs):
        ...
        assert kwargs['model'] == 'openai/tla-hd-mock'
    ...
    def settings(virtual_key, timeout=15):
        return Settings(_env_file=None, llm_mode='gateway', gateway_base_url=BASE+'/v1',
            gateway_virtual_key=virtual_key, gateway_model='tla-hd-mock',
    ...
    try:
        ...
        status, data = http('/key/generate', master, {'models':['tla-hd-mock'], 'duration':'1h',
            'key_alias':run_id, 'metadata':{'tags':['tla-hd']}, 'max_budget':1})
        ...
        except llm.litellm.AuthenticationError:
        ...
        except (llm.litellm.APIConnectionError, llm.litellm.Timeout, llm.litellm.InternalServerError) as exc:
            if isinstance(exc, llm.litellm.InternalServerError):
                assert 'Connection error' in str(exc), 'unexpected internal error during outage'
            assert len(report['transport_attempts']) == before+1
        ...
        for c in report['calls']:
            ...
            assert 'tla-hd' in r['request_tags'] and not r['canary_leaked']
