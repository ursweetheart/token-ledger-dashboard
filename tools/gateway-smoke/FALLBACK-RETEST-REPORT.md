...
[Fallback JSON](law-fallback-e3cee80a23d0.json): **20/20 stage PASS**; 18 lần gọi Gateway SDK; 9 direct-fallback mock attempts; 12 row dựng trong memory (5 gateway, 7 direct_fallback); 1 SMTP mock email. Hai direct attempts cố ý lỗi không tạo row thành công. Bao gồm LB dừng thật, request liên tiếp/đồng thời, fallback tắt, direct failure, phục hồi và outage lần hai. Không ghi app PostgreSQL, không Gmail/Google thật.

[LB hồi quy mới](law-lb-815c700c6af2.json): **PASS, 93 request thành công / 8 user / 93 SpendLogs**, 93 response ID duy nhất khớp user và token, cả hai Proxy có traffic. Concurrency 1 → 2 → 4 → 8; Proxy-down 8/8; recovery 4/4. Fallback tắt + LB-down: một SDK attempt, không bypass. Không phát hiện canary ở các trường log đã kiểm tra.
...
```bash
C:/law_insight/.venv-gateway/Scripts/python.exe -B tools/gateway-smoke/law_fallback_test.py
