"""Isolated real-SDK LB fault test; direct provider/SMTP are MOCK boundaries.

Run with C:/law_insight/.venv-gateway/Scripts/python.exe and an empty cwd.
...
def load_app():
    ...
    from app.services import llm, token_logger
...
async def exercise(report, app):
    ...
    class MockSMTP:
        ...
        def send_message(self, message, *args, **kwargs):
            ...
            messages.append(text)  # Actual rendered message retained in memory only.
    ...
    async def guarded_completion(**kwargs):
        ...
        return llm.litellm.ModelResponse(id='mock-direct-' + secrets.token_hex(8),
    ...
    async def invoke(label, helper, route=None, failure=False, missing=False, direct_failure=False):
        ...
        with contextlib.nullcontext() if missing else ledger.token_usage_context(**identity):
            try:
                ...
                error = type(exc).__name__  # Never store raw exceptions.
                if missing:
    ...
    try:
        ...
        with patch.object(llm, 'record_model_call', capture), \
            ...
            llm.settings = settings(credential='sk-invalid-synthetic-fallback-test')
            for helper in ('call_llm', 'call_llm_chat'):
...
def main():
    ...
    try:
        scratch = tempfile.mkdtemp(prefix='law-fallback-no-env-')
        os.chdir(scratch)
        try:
