"""Đo một đường gọi LLM, từng lượt một, rồi ghi lại đủ để tính lại về sau.
    ...
    python tools/diagnostics/do_duong_llm.py --duong gateway --khoa-bien CRM_VIRTUAL_KEY --thu-kho

    # hai lượt để bắt lỗi công cụ TRƯỚC khi tin số nó in ra (ô 1.5)
    python tools/diagnostics/do_duong_llm.py --duong gateway --khoa-bien CRM_VIRTUAL_KEY \
        --model gemini-2.5-flash --so-luot 2 --agent crm-feedback

    # đường gọi thẳng nhà cung cấp, để so với đường qua Gateway
    python tools/diagnostics/do_duong_llm.py --duong truc-tiep --khoa-bien KEY_BENCH_CRM_TEST \
        --model gemini-2.5-flash --so-luot 2 --agent crm-feedback
