...
- [x] 1.1 Xác nhận `git status` sạch trên `db/`, `scripts/`, `backend/` để quay lui được từng bước — **ĐO 07/09/2026: sạch.** `git status --porcelain db/ scripts/ backend/` trả về rỗng
- [x] 1.2 Xác nhận `docker compose ps` cho thấy `token-ledger-postgres` là `Up (healthy)` — **ĐO 07/09/2026: `Up (healthy)`.** Lưu ý cho lần sau: Docker Desktop bật KHÔNG có nghĩa là stack đang chạy — container lúc kiểm đang `Exited (0) 9 hours ago`, phải `docker start token-ledger-postgres` rồi `pg_isready` mới xanh
- [x] 1.3 Ghi lại 8 con số mốc từ SQLite hiện tại qua view `usage_resolved` (1.161 dòng · input 539.827.701 · output 87.459.865 · cached 227.679.097 · total 851.897.312 · 285,18 USD · khoảng 2026-01-01→2026-08-13 · nguồn billing 942/app 164/monitoring 45/NULL 10) vào file tạm để đối chiếu ở nhóm 4 — **ĐÓNG 07/09/2026: sản phẩm đã có, chỗ chứa thì không còn.** Tám con số nằm ngay trong chính dòng task này, tức việc ghi lại ĐÃ làm xong từ đầu, chỉ quên tick. Nhóm 4 cũng đã đối chiếu xong bằng chúng. Không còn file SQLite nào trong repo (`var/*.sqlite`, `db/*.sqlite` đều trống) nên không sinh lại được nữa — mà cũng không cần: change này chính là việc bỏ SQLite
- [x] 1.4 Xác nhận `python -m pytest` hoặc phép kiểm tương đương của `db/` (nếu có) và `node --test tests/date-range-filter.test.js` đang xanh — **ĐO 07/09/2026: vế JavaScript xanh 30/30**, cả 6 file `tests/*.test.js`. Trước đó `tests/date-range-filter.test.js:98` ĐỎ, và **lỗi nằm ở phép kiểm chứ không ở mã**: ba assertion ngay trên nó trong cùng test đều kiểm `dd/MM/yyyy`, riêng nó còn sót `08/13/2026` kiểu Mỹ cho ngày `2026-08-13`. Đã sửa kỳ vọng thành `13/08/2026` kèm ghi chú lý do — KHÔNG đụng vào mã sinh ra chuỗi đó. **Vế Python KHÔNG chạy được: `pytest` chưa cài** trong Python312 (`No module named pytest`), nên `tests/test_acceptance_safety.py` và `tests/test_connect_migrations.py` hiện không ai chạy. Mục này ghi "(nếu có)" nên không chặn việc đóng, nhưng đây là một lỗ hổng thật và nó nằm NGOÀI phạm vi change này: hai file kiểm tồn tại trong repo mà không lệnh nào trong dự án chạy được chúng. Cần một change riêng để cài `pytest` và nối nó vào bộ kiểm thường trực
