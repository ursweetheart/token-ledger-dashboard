# continuous-integration-checks Specification

## Purpose

Điều gì phải đúng trước khi code rời khỏi máy thì phải có máy canh, không giao cho trí nhớ.

Capability này nói về **lớp canh tự động chạy trên mỗi lần đẩy code** — nó chạy bộ kiểm nào, phải
chứng minh được gì, và chạy trong môi trường ra sao. Phân biệt với `automated-check-coverage`, vốn
nói về chất lượng của các phép kiểm **dữ liệu**; capability này nói về việc *có chạy hay không* và
*chạy ở đâu*.

Hai yêu cầu ở đây trông thừa nhưng không thừa, và cả hai đều sinh ra từ lỗi đã đo được, không phải
từ lý thuyết:

- **Đếm số phép đã chạy** thay vì tin mã thoát. Đo 13/09/2026: `node --test` với mẫu tên tệp không
  khớp cho ra 0 phép kiểm và mã thoát 0 — tức xanh mà chưa kiểm gì.
- **Chạy được trên máy sạch, không cài gói.** Đây là tính chất repo đang có thật, rẻ, và mất lúc nào
  không biết — chỉ cần một lời `import` đặt lên đầu tệp.

Bài học đắt nhất của change sinh ra capability này: một phép kiểm chưa từng đỏ không phải là phép
kiểm đã đạt, nó là phép kiểm chưa ai biết có chạy hay không. Hai lỗi nghiêm trọng nhất đều chỉ lộ ra
khi cố ý làm hỏng để xem có đỏ không — trong đó có một lỗi khiến cả hai nhóm kiểm báo **xanh trong
khi đang có phép kiểm trượt**.
