...
Toan bo lay tu thu muc data/, tuc nhung gi cac script pull_* da thu thap:
    ...
    data/raw_web/tla-hd/<ngay>/usage-day-user-model.json  <- pull_tla_contract_usage.py
...
---------------
    ...
    4  load_tla_contract        fact_app_daily
...
source='app'. Chi tiet o db/load_tla_contract.py.
"""
...
STEPS = [
    ...
    ("TLA Contract usage", "db/load_tla_contract.py", []),
    ...
    #
    # `--optional`: khong phai ngay nao cung co lan keo nha cung cap, va thieu
    # no KHONG phai loi. Nhung bo nap se noi to rang no thieu, kem hau qua.
    ("Provider ledger",  "db/load_provider.py",     ["--optional"]),
